...
import { FirstAMOTabSell, FirstCoverTabSell, FirstIcebergTabSell, FirstRegularTabSell, SecondAMOTabSell,  SecondIcebergTabSell, SecondRegularTabSell } from '../../component/screens/BuyScreen/SecondTabPages'

const SellScreen = ({ navigation }: { navigation: any }) => {
    ...
    const firstOptionRenderScene = SceneMap({
        first: FirstRegularTabSell,
        second: FirstCoverTabSell,
        third: FirstAMOTabSell,
        fourth: FirstIcebergTabSell
    });

    const secondOptionRenderScene = SceneMap({
        first: SecondRegularTabSell,
        second: SecondAMOTabSell,
        third: SecondIcebergTabSell,
                    ...
                    <View style={{ rowGap: 4 }}>
                        ...
                        <View style={{ flexDirection: 'row', columnGap: 12, alignItems: 'center' }}>
                            <View style={{ flexDirection: 'row', columnGap: 6 }}>
                                <CustomRadioButton activeColor={"#da494f"} innerColor={"#e7e7e7"} checkFlag={firstCheckState} onPressFunction={() => { setFirstCheckState(true); setSecondCheckState(false); }} />
                                <Text style={{ color: firstCheckState == true ? '#da494f' : 'gray', fontSize: 11 }}>NSE: ₹ 2,924.15</Text>
                            </View>
                            <View style={{ flexDirection: 'row', columnGap: 6 }}>
                                <CustomRadioButton activeColor={"#da494f"} innerColor={"#e7e7e7"} checkFlag={secondCheckState} onPressFunction={() => { setFirstCheckState(false); setSecondCheckState(true); }} />
                                <Text style={{ color: secondCheckState == true ? '#da494f' : 'gray', fontSize: 11 }}>BSE: ₹ 2,923.12</Text>
                            </View>
                        </View>
                    </View>
                </View>
...

            {
                firstCheckState == true && secondCheckState == false && <TabView
                    ...
                    renderTabBar={props => (
                        <TabBar
                            {...props}
                            indicatorStyle={{ backgroundColor: '#da494f' }}
                            tabStyle={{ width: "auto" }}
